...
```python
from omnisus_dbf import open_reader
...
```python
from omnisus_dbf import decompress_dbc
...
decoder in `omnisus.sources.datasus_ftp.dbc`. Both reject a declared DBF
header shorter than 33 bytes with the same message.

...
@app.cell
def _(agravos, resolve_target, mo, run_without_buttons):
    AGRAVO = "sinan_chagas"
    ANO = 2022
    EXECUTAR = False
...
@app.cell
async def _(AGRAVO, asyncio, executar, mo, sus, pl):
    mo.stop(
        not executar,
        mo.md(
    ...
    _publicados = await asyncio.to_thread(sus.available_releases, AGRAVO, refresh=True)
...
@app.cell
def _(AGRAVO, ANO, MAX_DOWNLOAD_BYTES, asdict, executar, mo, sus, save_plan, target):
    mo.stop(
        not executar,
        mo.md("Defina `EXECUTAR = True` para gravar o plano e importar."),
    )
    escopos = sus.scopes_for(AGRAVO, years=[int(ANO)])
...
def _(escopos, mo, sus, plano, reconcile, relatorio):
    with sus.LakeReader(plano["target"]) as _leitor:
        ...
        desatualizados = sus.outdated(plano["dataset"], lake=_leitor)
    {
